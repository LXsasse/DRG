...
# Instead of selecting a subset of cell types, we can also average cell types for attributions
	# This average can also be across all conditions for a data type, f.e. exonic, intronic, and degradation
	# If the names of the cell types are already data type specific in the output files, f.e. B.fo_kd, B.fo_ex, then the $classfile should also be data type specific, i.e B.fo_kd => B_kd
	# Otherwise if the the different output files have overlapping cell types names in them, the classes can be defined independent of the data type, e.g. B.fo => B 
