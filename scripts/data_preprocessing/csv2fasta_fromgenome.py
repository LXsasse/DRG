...
if '--random' in sys.argv:
    rand = np.random.permutation(len(ifile))[:int(sys.argv[sys.argv.index('--random')+1])]
    ifile = ifile[rand]
...
for u, uchr in enumerate(uchroms):
    if os.path.isfile(chrfold.rstrip('/')+'/'+uchr+'.fa.gz'):
        ...
        for e in chrmask:
            extseq = chrfasta[int(ifile[e, 2])-offset-flank+flank:int(ifile[e,3])-offset+flank+flank]
