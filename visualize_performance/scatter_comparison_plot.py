...
def readfiles(file1, delimiter = None, subtract = False):
    ...
    if header is None and ((len(vals1[0]) != len(vals1[1])) or ( not checknum(vals1[0][0]))):
        ...
        vals1 = vals1[1:]
        names1 = names1[1:]
    vals1 = np.array(vals1, dtype = float)
    if subtract:
        vals1 = 1-vals1
...
if __name__ == '__main__':    
    ...
    if '--columns_and_row' in sys.argv:
        ...
        names2 = names2[srow2]
        vals1 = vals1[srow1, scol1]
        vals2 = vals2[srow2, scol2]
    ...
    if '--sizefile' in sys.argv:
        ...
        snames, sizes = readfiles(sizefile,scol)
        if np.isin(snames, names).all():
            sort = np.argsort(snames)
            sizes = sizes[sort]
        else:
