...
def read_file(f, expdelim = None):
    f = open(f, 'r').readlines()
    experiments = f[0][1:].strip().replace('"','').split(',')
...
def vulcano(fc, pv, figname, xcutoff=1., ycutoff = 1.96, text = None, mirror = False, eigenvalues = False):
...
expfc, genfc, valfc = read_file(sys.argv[1], expdelim = '.FC')
exppv, genpv, valpv = read_file(sys.argv[2], expdelim = '.PV')
#print expfc, exppv
...
for e, exp in enumerate(expfc):
    ...
    if '--onlylist' in sys.argv:
        sigs = np.where((np.absolute(np.log2(valfc[:,e])) > foldcut) & (valpv[:,d]>pcut))[0]
        siggenes.append(sigs)
    else:
        fi = vulcano(np.log2(valfc[:,e]), valpv[:,d], exp, mirror = mirror, eigenvalues = eigvals) #, text = genfc)
